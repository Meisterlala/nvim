...
function M.decrypt(password)
  ...
  -- Use openssl for decryption (base64 + explicit params for cross-platform compatibility)
  local cmd = string.format(
    'openssl enc -%s -d -a -pbkdf2 -iter 100000 -in %s -out %s -pass pass:%s 2>&1',
    CIPHER,
...
function M.encrypt(new_password)
  ...
  -- Use openssl for symmetric encryption (base64 + explicit params for cross-platform compatibility)
  local cmd = string.format(
    'openssl enc -%s -salt -a -pbkdf2 -iter 100000 -in %s -out %s -pass pass:%s 2>&1',
    CIPHER,
    vim.fn.shellescape(DECRYPTED_FILE),
    vim.fn.shellescape(ENCRYPTED_FILE),
    vim.fn.shellescape(password)
