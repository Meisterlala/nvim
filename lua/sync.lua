local function commit_and_push_config_changes()
  ...
  local message = 'Neovim config update: ' .. timestamp
  local commit_command = 'git commit -m "' .. message .. '"'
  local commit_handle = io.popen(commit_command .. ' 2>&1')
  local commit_output = commit_handle:read '*a'
  local commit_success = commit_handle:close()
  ...
  local push_success = push_handle:close()
...
vim.keymap.set('n', '<leader>gc', ':ConfigPush<CR>')
